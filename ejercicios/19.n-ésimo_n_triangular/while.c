...
int calcularNumeroTriangular(int n) {
    ...
    int contador = 1; // Inicialización del contador para el bucle while

    // Bucle para sumar los números naturales hasta n
    while (contador <= n) {
        numeroTriangular += contador; // Suma el número actual al número triangular
        contador++; // Incrementa el contador para avanzar al siguiente número
    }
