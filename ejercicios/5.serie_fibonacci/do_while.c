...
int main() { // Función principal del programa
    // Declarar variables
    int n, first = 0, second = 1, next, count = 0; // Variables para la serie de Fibonacci y el control de bucle
    ...
    do { // Repetir hasta que se impriman n elementos
        if (count == 0) { // Verificar si es el primer elemento
            printf("%d", first); // Imprimir el primer elemento
        } else if (count == 1) { // Verificar si es el segundo elemento
        ...
        count++; // Incrementar el contador de elementos impresos
    } while (count < n); // Verificar si se han impreso todos los elementos
